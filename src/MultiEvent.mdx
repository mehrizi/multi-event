...
import { Title, Subtitle, Description, Primary, Controls, Stories,Source,Story } from '@storybook/blocks';
...
### How to use
It is important to remember that `<MultiEvent>` Component fills the parent width. It adjusts the height acoordingly.
In this example the width is fixed to 300px
<Canvas of={CalendarStory.Bounded} Source layout='padded' />

The Following code is simplest way of integrating the calendar
```JSX
